...
# Source directory
src_dir="/home/administrator/frappe-bench/sites/erp.local/private/backups"
...
if [ -n "${latest_pair[0]}" ]; then
  ...
  #cp "${latest_pair[0]}".sql.gz "${ext_drive}"
  check_file_size "${latest_pair[0]}"
else
  echo "No .sql.gz files found in the source directory."
fi

#Sort corresponding json file in the source directory by creation time and copy latest file
latest_pair_two=($(ls -1t "$src_dir"/*.json | head -n 1))
if [ -n "${latest_pair_two[0]}" ]; then
  cp "${latest_pair_two[0]}" "${ext_drive}"
 # cp "${latest_pair_two[0]}".json "${ext_drive}"
  check_file_size "${latest_pair_two[0]}"
else
  echo "No .json files found in the source directory"
fi
